...
@interface AAPLCameraViewController () </*AVCaptureFileOutputRecordingDelegate,*/AVCaptureAudioDataOutputSampleBufferDelegate>{
...
@property (weak, nonatomic) IBOutlet UILabel *randomDigital;
@property (weak, nonatomic) IBOutlet UILabel *digitalQueue;
...
@property (nonatomic) NSMutableArray *digitalArray;
//@property (nonatomic) AVCaptureMovieFileOutput *movieFileOutput;
@property (nonatomic) AVCaptureStillImageOutput *stillImageOutput;
@property (nonatomic) AVCaptureVideoDataOutput *videoDataOutput;
@property (nonatomic) AVCaptureAudioDataOutput *audioDataOutput;
@property (nonatomic) AVCaptureConnection	*videoConnection;//视频录制连接
...
@property (strong, nonatomic) FlowerWriter *flowerWriter;
...
//	[_recordButton setb]
	...
	dispatch_async( self.sessionQueue, ^{
		...
		/***写入流****/
		self.flowerWriter =  [[FlowerWriter alloc] initWithVideoOutput:self.videoDataOutput audioOutput:self.audioOutput userName:self.userName];
		
        [self.videoDataOutput setSampleBufferDelegate:self.flowerWriter queue:dispatch_get_main_queue()];
        [self.audioOutput setSampleBufferDelegate:self.flowerWriter queue:dispatch_get_main_queue()];
...
//		}
        ...
        if (!weakSelf.flowerWriter.isRecording) {
            ...
            [weakSelf.flowerWriter startWriting];
			
			[weakSelf.flowerWriter startAudioSingleWriting];
...
//             [weakSelf.session startRunning];
            ...
            [weakSelf.flowerWriter stopWriting];
			
			[weakSelf.flowerWriter stopAudioSingleWriting:NO];
...
#pragma mark --获得音频存放地址
//获得音频存放地址
- (NSString *)getAudioCachePath {
    NSString *aduioCache = [NSTemporaryDirectory() stringByAppendingPathComponent:@"audios"] ;
    BOOL isDir = NO;
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL existed = [fileManager fileExistsAtPath:aduioCache isDirectory:&isDir];
    if ( !(isDir == YES && existed == YES) ) {
        [fileManager createDirectoryAtPath:aduioCache withIntermediateDirectories:YES attributes:nil error:nil];
    };
    return aduioCache;
}
...
//    _randomDigital.text = [NSString stringWithFormat:@"%d",arc4random() % 10]; // 10以内的数字
//    _digitalQueue.text = _randomDigital.text;
    _timer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(refrushDigital) userInfo:nil repeats:YES];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([_randomDigital.text isEqualToString:@" "]) {
				...
				if (self.count > 1) {
					__weak typeof (self) weakSelf = self;
					dispatch_async( self.sessionQueue, ^{
						[weakSelf.flowerWriter stopAudioSingleWriting:YES];
					} );
            ...
            _digitalQueue.text = [NSString stringWithFormat:@"%@%@",_digitalQueue.text,_randomDigital.text];
			self.count++;
        });
